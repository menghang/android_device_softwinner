...
static int display_setareapercent(struct display_device_t *dev,int displayno,int percent)
{
    ...
    ctx->valid_width[displayno]    = display_getvalidwidth(ctx,displayno,ctx->out_type[displayno],ctx->out_format[displayno]);
    ctx->valid_height[displayno]   = display_getvalidheight(ctx,displayno,ctx->out_type[displayno],ctx->out_format[displayno]);
    ctx->valid_width[displayno] = (ctx->area_percent[displayno] * ctx->valid_width[displayno]) / 100;
    ctx->valid_height[displayno] = (ctx->area_percent[displayno] * ctx->valid_height[displayno]) / 100;
...
static int display_setorientation(struct display_device_t *dev,int orientation)
{
    ...
    if(ctx->mode == DISPLAY_MODE_SINGLE_VAR_GPU)
    {
        ...
        ctx->valid_width[0]    = display_getvalidwidth(ctx,0,ctx->out_type[0],ctx->out_format[0]);
        ctx->valid_height[0]   = display_getvalidheight(ctx,0,ctx->out_type[0],ctx->out_format[0]);
...
static int display_setmode(struct display_device_t *dev,int mode,struct display_modepara_t *para)
{
    ...
    if(mode == DISPLAY_MODE_DUALSAME)
    {
        ...
        ctx->valid_width[0]    = display_getvalidwidth(ctx,0,para->d0type,para->d0format);
        ...
        ctx->valid_width[1]    = display_getvalidwidth(ctx,1,para->d1type,para->d1format);
        ctx->valid_height[1]   = display_getvalidheight(ctx,1,para->d1type,para->d1format);
        ctx->valid_width[0] = (ctx->area_percent[0] * ctx->valid_width[0]) / 100;
        ctx->valid_height[0] = (ctx->area_percent[0] * ctx->valid_height[0]) / 100;
        ctx->valid_width[1] = (ctx->area_percent[1] * ctx->valid_width[1]) / 100;
        ctx->valid_height[1] = (ctx->area_percent[1] * ctx->valid_height[1]) / 100;
    ...
    if(init_para.b_init)
    {
        ...
        for(sel=0; sel<2; sel++)
        {
                    ...
                    || (init_para.disp_mode == DISP_INIT_MODE_TWO_SAME_SCREEN))))
            {
                ...
                ctx->valid_width[sel] = display_getvalidwidth(ctx, sel, ctx->out_type[sel], ctx->out_format[sel]);
                ctx->valid_height[sel] = display_getvalidheight(ctx, sel, ctx->out_type[sel], ctx->out_format[sel]);
        ...
        struct hw_device_t** device)
{
    ...
    {
        status = errno;
        LOGE("Error opening display errno=%d (%s)",
